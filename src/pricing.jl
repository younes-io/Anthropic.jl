...
function call_cost(input_tokens::Int, output_tokens::Int, cache_creation_input_tokens::Int, cache_read_input_tokens::Int, model::String)
    if !haskey(TOKEN_PRICES, model)
        @warn "Model $model not found in TOKEN_PRICES. Using default costs."
        return 0.0
    end
    
    costs = TOKEN_PRICES[model]
...
call_cost!(meta::StreamMeta, model::String) = (meta.cost = call_cost(meta.input_tokens, meta.output_tokens, meta.cache_creation_input_tokens, meta.cache_read_input_tokens, model))
...
function format_meta_info(meta::StreamMeta)
    ...
    meta.cost > 0 && push!(parts,  "\$$(round(meta.cost, digits=6))")
